...
///
/// 取值须高于**合法**静默间隔的上限，且本超时不应成为杀流的那一方。
///
/// 实测（trace_phases.detail 的 idle_ms）：上游 AWS 对单条流有约 240s 的静默上限，
/// 四次中断落在 239707~239996ms、彼此相差不足 300ms，是明确的服务端定时器；
/// 且 h2 PING 保活**不能**重置它——0.8.5（已带保活）仍出现 239707ms 一次。
/// 原因：SOCKS5 是 L4 隧道、TLS 端到端，RST_STREAM（`stream error received:
/// unexpected internal error`）来自 TLS 对端即上游本身，非链路中间设备。
///
/// 另有一次合法静默达到 305821ms（0.8.3，无任何本地超时干预），因此 300s 一类的
/// 取值会在上游之前亲手掐死正常请求。600s 取「已观测最长合法静默的约 2 倍」：
/// 上游若要杀流，其 RST 必先到（错误更可归因）；本超时只兜底「连接活着、PING 有
/// 回应、上游既不发数据也不发 RST」的真卡死。传输层真死由 h2 PING（25s 间隔 +
/// 15s ACK 超时）约 40s 识别，不依赖本超时。
const STREAM_IDLE_TIMEOUT_SECS: u64 = 600;

/// 编译期钉住取值下界：已观测到 305821ms 的**合法**静默，调到它以下就会把正常的
/// 长思考误判为卡死，正是本次改动要消除的失败形态。放编译期而非测试里，是因为
/// 这是常量的不变式，改错时应当直接编不过。
const _: () = assert!(STREAM_IDLE_TIMEOUT_SECS > 306);
