...
mod tests {
    ...
    /// 0.9.17 起「全员不可调度」的文案改为分类计数（禁用 / 冷冻 / 冷却）。
    /// 归一化不需要新增规则——通用的「数字换 N」已经把计数收敛掉；这里钉住
    /// 两件事：① 同一类告警的不同计数收敛为一条；② **它与旧文案是两个独立
    /// 指纹**，全员冷冻（等一等就自己好）与真·全禁用（要人工介入）不能被
    /// 合并成同一条头部告警。旧文案的规则保留在上一条测试里，不删。
    #[test]
    fn unschedulable_classified_message_has_its_own_fingerprint() {
        let (_, a) = normalize_error_message(
            "所有凭据不可调度（禁用 0 / 冷冻 3 / 冷却 0，共 3），最早解冻 08-22 05:20",
        );
        let (_, b) = normalize_error_message(
            "所有凭据不可调度（禁用 1 / 冷冻 2 / 冷却 0，共 3），最早解冻 09-01 11:07",
        );
        assert_eq!(a, b, "计数与时间应被归一化掉");
        let (_, legacy) = normalize_error_message("所有凭据均已禁用（3/3）");
        assert_ne!(a, legacy, "新旧文案必须是两个独立指纹");
    }
