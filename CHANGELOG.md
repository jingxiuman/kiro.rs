...
### 🛡️ 主动余额护栏（此前只有被动的 402 兜底）

- **根因**：超额→冷冻此前只有两条被动路径——上游回 402，或人工点面板「一键超额」。而 402 恰恰只在上游 **overage 关闭**时才会出现：开了 overage 的账号额度用尽后上游不拒付、直接计费，402 那条链路永远不触发。护栏建在了被保护对象失效时才会响的信号上。生产实测：8 张凭据里 6 张 `overageEnabled=true`，其中 #10 一路透支到 `remaining = -202.59`（usage 102%）才被人工禁用。
- **判据**：新增配置 `quotaGuardReserve`（默认 `200.0`，`0` = 退化为改造前的「remaining ≤ 0 才算超额」）。有效剩余低于该值即视为超额。**留垫而非取 0** 是因为余额快照最长可陈旧 `MAX_STALE_SECS`（3600s）、本地消耗计数只覆盖本进程，「余额显示归零」时实际早已透支——#10 那 202 就是这段观测延迟。
- **落点一（持久）**：被动余额刷新拿到 `remaining` 后调 `apply_balance_observation`，跌破安全垫即冷冻退池，高于安全垫则提前解冻。冻与解冻收敛到同一个方法、共用同一个判据函数 `quota_exhausted`——分开写就要各判一次界，界不一致时安全垫区间 `(0, reserve]` 内两边同时成立，每轮刷新都会冻一次解一次并各写一次盘。
- **落点二（实时）**：选号候选过滤新增一条，按「缓存余额 − 本代次消耗」拦截，能在两次刷新之间就挡住正在被打穿的凭据。**同时补在 priority 模式的 `current_id` 快路径上**：那条快路径走 `entry_schedulable`，而该函数看不见余额，只加候选过滤会留下一条旁路（测试 `quota_guard_blocks_current_id_fast_path_in_priority_mode` 在实现前确实选中了低余额凭据）。两处共用 `quota_guard_blocks`。
- **全组超额直接报错**，不降级放行——放行就是继续花钱。走的是既有的「无可调度凭据」路径，`dispatch::pick` 的 I1 降级契约不受影响。
- **「不知道」不等于「已耗尽」**：没有可用余额快照的凭据一律放行，交给 402 兜底。把缺快照当成耗尽会把新登录/刚导入的凭据永久锁在池外——0.9.18 那条 profileArn 自锁（无 ARN → 查不到余额 → 权重恒 0 → 永不被调度 → 永不解析 ARN）就是这个形状。
- **文案**：`unschedulable_message` 新增「额度不足」一档（优先级 禁用 > 冷冻 > 额度不足 > 冷却）。少了它，「全员被护栏挡下」会打出三类全零却报全员不可用的自相矛盾消息，即仓库里 "9/1" 那次事故的同形态。
- **不在本次范围**：不改动上游 overage 设置（6 张 `overageEnabled=true` 维持原样），护栏纯在本地；面板「一键开启超额」按钮保留。
- **已知窗口**：落点二挡下但尚未被刷新冻上的凭据，在面板上暂时既非禁用也非冷冻，看起来「正常但不被调度」。下一次被动刷新（≤300s）会把它写成冷冻，届时面板一致。
- **测试**：980 → 986。clippy 109 → 108（无新增；service.rs 一处嵌套 if 被合并消掉）。
...
- **面板「一键超额」同步改为冷冻**：该动作此前写 `disabled + QuotaExceeded`，依赖的「刷新探测到额度恢复就重新启用」自愈链路已随本次改造删除，会退化成没有出口的死禁用。现与 402 走同一条路径、同一套截止时间规则（`next_reset_at + 300s`，拿不到则兜底 1h）。路由 `/credentials/disable-quota-exceeded` 与响应字段 `disabledIds` 为对外契约保持不变。
- **存量迁移（覆盖范围如实说明）**：加载时把 `disabled + disabledReason=QuotaExceeded` 的凭据转为冷冻。但 `disabledReason` 是**本版本才开始持久化**的字段——0.9.16 的 `credentials.json` 里没有它，旧文件读出来一律是 `Manual`，因此**真实的 0.9.16→0.9.17 升级路径走不到这条迁移**。它实际覆盖的是「C2 修复前的一键超额写出的持久化 QuotaExceeded 数据」以及手工编辑出的同形数据。
- **升级须知**：升级前已被 402 禁用、且此后重启过的凭据（reason 已丢失为 `Manual`）需要**在面板手动启用一次**；启用后若额度仍未恢复，会立即吃一次 402 并自动进入冷冻，此后无需人工干预。（这不是本次引入的回归：旧版本的禁用原因本就只存在于内存，重启后旧的刷新自愈链路同样救不回这批凭据。）
- **`frozen_until` 上界钳制**：三处写入点（402 触发、一键超额、`refine_freeze_deadline`）与迁移路径统一 `min(now + 35 天)`，防上游 `next_reset_at` 的时间戳量纲变化（秒→毫秒）把凭据冻到天荒地老——那是一种无告警、只表现为「这张号再也不被选中」的静默失效。
...
- **全员冷冻的错误文案与 ops 指纹**：`所有凭据均已禁用（3/3）` 在全员冷冻时既自相矛盾（可用数等于总数却说全禁用），又与真·全禁用共用同一个 ops 指纹。改为分类计数 `所有凭据不可调度（禁用 D / 冷冻 F / 冷却 T，共 N）`，`F > 0` 时附带最早解冻的本地时间。归一化无需新规则（通用的数字换 `N` 已收敛计数），旧文案规则保留，新增测试钉住"新旧是两个独立指纹"。**选号路径与 token 刷新失败路径共用同一条文案**（`unschedulable_message`）：后者原本硬编码 `所有凭据均已禁用（0/N）`，而「剩余凭据全冷冻」现在也会走到它——同一个故障会按"是否顺带撞上 token 刷新失败"分裂成两个指纹，其中旧文案那半还会被归一进要人工介入的旧桶。
- **粘滞会话归类**：weighted 选号里冷冻凭据的排除原因从 `Transient` 改为 `Durable`——冷冻按天计（截止通常是下一个月度重置），粘滞会话与其在这么长的窗口里反复抖动，不如一次性迁走。
- **面板三态优先级**：凭据卡片的 已禁用 > 冷冻 > 冷却 互斥显示，被禁用的凭据不再同时挂冷冻徽标与「立即解冻」菜单项。
