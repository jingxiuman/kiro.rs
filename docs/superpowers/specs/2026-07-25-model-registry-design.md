...
>
> **四次修正（终版）：比例护栏不得自动放行，歧义交人工确认。**
>
> 前三版都试图用一条自动规则区分「探针不具代表性」与「上游真实大批退役」。这条路走不通，原因是**两者在数据上完全无法区分**：
>
> | | 稳定误配的探针 | 真实的大批退役 |
> |---|---|---|
> | 单轮缺失比例 | 高 | 高 |
> | 连续多轮的缺失集合 | **稳定**（误配是持久状态） | **稳定**（模型确实没了） |
>
> 因此「连续 N 轮一致就放行」这类规则，修好合法退役的同时，必然让永久误配的探针在 N 轮后照样刷掉整表 —— 只是把误杀推迟了 N 轮。第三版的 Active 口径修正也没有解决原始症状：实测「上游一次性只保留 5/13」连跑 20 个权威轮，计数器一次都没递增（该场景下所有行都是 Active，比例恒定，护栏形成不动点）。
>
> **终版设计：护栏触发时暂停消失判定，并明确告知，等人确认。**
> - 本轮的新增/更新照常进行，只跳过消失判定。
> - `SyncSummary` 必须新增 `disappearanceCheckSkipped: bool` 与 `missingRatio: f64`，透传到 `POST /models/sync` 的响应与 admin UI。**没有这一条，系统会长期处于「同步天天成功、消失判定其实已停机」而无人知晓的状态。**
> - 提供显式的运维出口：运维确认探针凭据无误后，可手动触发一次**强制放行**的同步，该轮正常执行消失判定。
> - admin UI 在护栏触发时显示横幅，说明两种可能（探针配错 / 上游真下线）并提供确认按钮。
>
> 这个设计承认了系统的认识边界：**分不出来的时候就说分不出来，而不是猜。** 代价是真实的大批退役需要人点一次确认 —— 这个代价是可接受的，因为大批退役本身就是罕见且值得人过目的事件。
